...
# file path 
test_id_file =  test_dir+'/testing_data/id.txt'
...
#         embedded = F.selu(embedded)
    ...
    def testing(self, video_seq, index2word, use_beam_search, beam_size):
        ...
        if use_beam_search:
            for step in range(self.output_step):
                if step == 0:
                    ...
                    for j, candidate in enumerate(candidates):
                        decoder_input = Variable(torch.LongTensor([candidate[-1]])).resize(1,1)
                        decoder_input = self.embedding(decoder_input)
            ...
            pred = [index2word[int(wix)] for wix in candidates[0] if int(wix) >= 3 ]
        else:
            for step in range(self.output_step):
                if step == 0:
                    decoder_input = init_BOS
                ...
                #print(word2ix, "--", word2ix.item())
                ix2word = index2word[word2ix.item()]
                if word2ix < 3:
                    break
                else:
                    pred.append(ix2word)
...
test_label_file = test_dir+'/testing_label.json'
test_frames = {}
test_label = pd.read_json(test_label_file).set_index('id')
for index, row in test_label.iterrows():
    test_frames[index] = torch.FloatTensor(np.load(test_dir+'/testing_data/feat/' + index + '.npy'))
